...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public User register(UserRegisterForm userRegisterForm) {
        ...
        queryWrapper.eq("login_name", userRegisterForm.getLoginName());//eq；where查询
        ...
        //存储数据
        User user = new User();
        BeanUtils.copyProperties(userRegisterForm, user); //封装对象类型转化
        user.setPassword(MD5Util.getSaltMD5(user.getPassword())); //密码加密
